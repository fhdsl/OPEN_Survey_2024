...
`r data.frame(df$GenSuggestions[df$GenSuggestions %>% str_detect("meeting time|conflict")]) %>% drop_na() %>% nrow()` out of `r nrow(df %>% select(GenSuggestions) %>% drop_na())` the general suggestions/comments mentioned a time conflict.
